...
class Motor():
    ...
    # TODO: add a stop condition to each use of this function
    def change_direction(self, stop_conditions, direction=None, back_rotations=0.5, steer_degrees=180, block=False):
        if direction is None:
            direction = random.choice([RIGHT] + [LEFT] * 9)
        
        
        self.run(forward=False, rotations=back_rotations, block=block)

        while not any(stop_conditions.values()) and self.motor.is_running:
            pass
        if not any(stop_conditions.values()):
            self.motor.on_for_degrees(direction * 100, SpeedPercent(self.base_speed), steer_degrees, block=block)
